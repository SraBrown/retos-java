...
public class StringHelper {
	...
	while (toPos != -1) {
	    stringArray.add(afterDecode(str.substring(fromPos, toPos)));
	    fromPos = toPos + 1;
	    toPos = str.indexOf(SEPARATOR, fromPos);
	}
	stringArray.add(afterDecode(str.substring(fromPos)));

	return (String[]) stringArray.toArray(new String[stringArray.size()]);
    }

    private static Object afterDecode(String substring) {
	...
	if (stringArray.length > 0) {
	    sb.append(beforeEncode(stringArray[0]));
	    for (int i = 1; i < stringArray.length; ++i) {
		sb.append(SEPARATOR);
		sb.append(beforeEncode(stringArray[i]));
	    }
	}
	return sb.toString();
    }

    private static Object beforeEncode(String string) {
		// TODO Auto-generated method stub
		return null;
	}
